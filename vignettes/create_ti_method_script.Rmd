...
dyncli helps creating a standalone Docker container for trajectory inference (TI) methods using dynwrap by auto-generating CLI and input/output file management based on the TI method's functionality.yml. In this vignette, we will go through the requirements for using dyncli. 
...
# g <- dynplot::plot_heatmap(trajectory, expression = dataset$expression)
# ggplot2::ggsave(filename = "heatmap.png", g, width = 6, height = 4)
```

```{r}
# install dynplot to plot the output
# dynplot::plot_heatmap(trajectory, expression = dataset$expression)
```
![](heatmap.png)
